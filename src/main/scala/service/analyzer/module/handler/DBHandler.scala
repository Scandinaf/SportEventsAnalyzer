...
/**
  ...
  */
protected[module] trait DBHandler[T <: MongoObject] extends Handler[T] with Logger {
  val dao: BaseDAOComponent[T]

  def handle(obj: T) =
    dao.insert(obj).subscribe((err: Throwable) => logger.error(err.toString), () => logger.info("OK"))
}
